...
class TestMetricsCollector:
    ...
    async def test_query_metrics_by_time_range(self, metrics_collector):
        ...
        await metrics_collector.record_metric(
            "test_metric", 30.0, MetricType.GAUGE, timestamp=current_time
        )  # now
    ...
    async def test_query_metrics_by_labels(self, metrics_collector):
        ...
        await metrics_collector.record_metric(
            "requests", 100, MetricType.COUNTER, {"service": "api", "status": "200"}
        )
        await metrics_collector.record_metric(
            "requests", 10, MetricType.COUNTER, {"service": "api", "status": "404"}
        )
        await metrics_collector.record_metric(
            "requests", 50, MetricType.COUNTER, {"service": "web", "status": "200"}
        )
    ...
    async def test_export_json(self, metrics_collector):
        ...
        # Record test metrics
        await metrics_collector.record_metric(
            "test_metric", 42.0, MetricType.GAUGE, {"host": "server1"}
        )
