...
class TestWritePatternAnalysis:
    ...
    def test_suspicious_write_pattern(self, monitor, test_file):
        ...
        time_sequence = [base_time + i * 0.01 for i in range(5)]  # 5 timestamps 0.01s apart
...
class TestErrorHandling:
    def test_handle_access_error(self, monitor, test_file):
        ...
        def stat_error(*args, **kwargs):
            raise PermissionError("Access denied")
        
        # Mock os.stat to always raise PermissionError
        with patch("os.stat", side_effect=stat_error):
            # Trigger a check cycle and expect FileAccessError
            try:
                monitor._check_files()
    ...
    def test_thread_safety(self, monitor, test_file):
        ...
        for i in range(5):
            thread = threading.Thread(
                target=lambda x: open(
                    os.path.join(monitor.repo_path, test_file), "w", encoding="utf-8"
                ).write(f"def test_{x}():\n    return True\n"),
                args=(i,)
            )
