...
class TestSSEEndpointIntegration:
    ...
    @pytest.mark.asyncio
    @patch("aider_mcp_server.pages.application.app._create_sse_monitoring_endpoint")
    async def test_sse_endpoint_headers(self, mock_create_sse_monitoring_endpoint, test_app):
        """Test that SSE endpoints return correct headers without full streaming."""

        # Mock the entire endpoint creation to return a simple response without event loop binding issues
        import uuid

        from starlette.responses import Response

        def mock_endpoint_creator(event_type: str):
            # Generate a client ID like the real implementation
            client_id = str(uuid.uuid4())

            # Return a simple Response with SSE headers, mimicking what EventSourceResponse would return
            headers = {
            ...
            return Response(content="", media_type="text/event-stream", headers=headers)

        mock_create_sse_monitoring_endpoint.side_effect = mock_endpoint_creator
