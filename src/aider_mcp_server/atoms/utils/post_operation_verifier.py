...
class VerificationResults:
    ...
    def __post_init__(self) -> None:
        ...
        self.warning_files = sum(1 for r in self.file_results.values() if r.overall_result == VerificationResult.WARNING)
        self.skipped_files = sum(1 for r in self.file_results.values() if r.overall_result == VerificationResult.SKIPPED)
...
class PostOperationVerifier:
    ...
    def _verify_single_file(self, file_path: str, baseline_metrics: Dict[str, Any]) -> FileVerificationMetrics:
        ...
        metrics.line_count_valid = self._verify_line_count(
            metrics.current_line_count, 
            metrics.baseline_line_count, 
            file_path
        )
        if not metrics.line_count_valid:
            reduction_pct = ((metrics.baseline_line_count - metrics.current_line_count) 
                           / max(metrics.baseline_line_count, 1)) * 100
    ...
    def _verify_line_count(self, current_count: int, baseline_count: int, file_path: str) -> bool:
        ...
        logger.debug(
            f"Line count validation passed for {file_path}: "
    ...
    def _verify_json_semantics(self, content: str, file_path: str) -> bool:
        ...
        try:
            ...
            raise SemanticVerificationError(f"Invalid JSON: {e}")
    ...
    def _verify_yaml_semantics(self, content: str, file_path: str) -> bool:
        ...
        try:
            ...
            raise SemanticVerificationError(f"Invalid YAML: {e}")