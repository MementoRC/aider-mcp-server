...
class SafetySystemManager:
    ...
    def _initialize_components(self, working_dir: Union[str, Path]) -> None:
        ...
        if self.config.enable_git_checkpoint and not self._git_checkpoint_manager:
            try:
                ...
                # In test environments or non-git directories, gracefully handle the error
    ...
    def pre_execution_check(
        ...
        try:
            ...
            # Risk assessment - simplified for integration
            risk_assessment = None
            if self.config.enable_risk_assessment and self._risk_assessor:
                try:
                    # Use a simplified risk assessment approach
                    ...
                    # Create a simple risk assessment object
                    risk_assessment = type('RiskAssessment', (), {
                        'risk_level': RiskLevel.CRITICAL if risk_score >= 4 else RiskLevel.MEDIUM if risk_score >= 2 else RiskLevel.LOW,
                        'total_score': risk_score,
                        'factors': [f"file_count={file_count}", f"prompt_length={prompt_length}"]
                    })()
                    
                    # Block high-risk operations in maximum safety mode
                    if (self.config.safety_level == SafetyLevel.MAXIMUM and 
                        ...
                        return PreExecutionResult(
                            ...
                            details={"risk_assessment": {"score": risk_assessment.total_score, "factors": risk_assessment.factors}},
                            user_message=(
            ...
            # File integrity pre-check - simplified for integration
    ...
    def create_checkpoint(
        ...
        try:
            ...
            if not self._git_checkpoint_manager:
                ...
                return CheckpointResult(success=True, checkpoint_id="unavailable", error_message="Git checkpoint not available")
    ...
    def generate_recovery_guidance(
        ...
        try:
            ...
            guidance_result = SimpleNamespace(
                ...
                critical_count=1 if hasattr(failure_detection, 'has_failures') and failure_detection.has_failures else 0,
                high_count=1,
                medium_count=1,
                low_count=0,
