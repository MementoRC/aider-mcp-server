...
class FileMonitor:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized FileMonitor for {repo_path} "
            f"(interval={check_interval}s, threshold={size_change_threshold})"
        )
            ...
            try:
                ...
                if stats.st_mtime > metrics.last_modified:
                    ...
                    if metrics.consecutive_writes > self.max_consecutive_writes:
                        logger.warning(
                            f"Suspicious write pattern detected for {file_path}: "
                            f"{metrics.consecutive_writes} consecutive writes"
                        )
                        self.state = MonitoringState.ERROR
                        raise WritePatternError(
                            f"Too many consecutive writes to {file_path}"
                        )
                    ...
                    if size_change > self.size_change_threshold:
                        logger.warning(
                            f"Suspicious size change in {file_path}: "
                            f"changed by {size_change:.1%}"
                        )
