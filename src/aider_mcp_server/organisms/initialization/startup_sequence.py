...
class StartupSequence:
    ...
    async def initialize_application(
        self,
        transport_configs: Optional[Dict[str, Dict[str, Any]]] = None
        ...
        try:
            ...
            coordinator = await asyncio.wait_for(
                self._perform_initialization(transport_configs),
        ...
        try:
            ...
            for transport_name, transport_config in transports_config.items():
                if isinstance(transport_config, dict) and transport_config.get("enabled", False):
                    await self._configure_single_transport(
                        coordinator, transport_name, transport_config
                    )
