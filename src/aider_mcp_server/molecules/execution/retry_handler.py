...
class FixedBackoffCalculator(BackoffCalculator):
    ...
    def calculate_delay(self, attempt: int, base_delay: float) -> float:
        ...
        if self.jitter:
            # Add ±25% jitter
            jitter_factor = 0.75 + (secrets.randbelow(500) / 1000)  # 0.75 to 1.25
            delay *= jitter_factor
        return delay
...
class RetryHandler:
    ...
    def __init__(
        ...
        **strategy_kwargs: Any,
        ...
        self._logger.info(
            f"RetryHandler initialized: max_retries={max_retries}, strategy={strategy.value}, base_delay={base_delay}s"
        )
        ...
        **kwargs: Any,
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                self._logger.warning(f"Function failed on attempt {attempt + 1}: {type(e).__name__}: {e}")
        ...
        **kwargs: Any,
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                self._logger.warning(f"Async function failed on attempt {attempt + 1}: {type(e).__name__}: {e}")
