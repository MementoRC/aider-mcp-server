...
class ResourceManager:
    ...
    async def cleanup_idle_resources(self) -> int:
        ...
        for resource in self._resources.values():
            ...
            if (self.constraints.max_idle_time is not None and 
                resource.state == ResourceState.IDLE and
                current_time - resource.last_accessed > self.constraints.max_idle_time):
                should_cleanup = True
            
            # Check age constraint
            if (self.constraints.max_age is not None and
                current_time - resource.allocated_at > self.constraints.max_age):
                should_cleanup = True
            
            # Check access count constraint
            if (self.constraints.max_access_count is not None and
                resource.access_count > self.constraints.max_access_count):
                should_cleanup = True
    ...
    def _can_allocate(self) -> bool:
        ...
        if (self.constraints.max_resources is not None and
            len(self._resources) >= self.constraints.max_resources):
            return False
        
        # Check allocator-specific constraints
        for resource_type, allocator in self._allocators.items():
