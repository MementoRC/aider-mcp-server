...
class ResponseFormatter:
    ...
    def process_coder_results(
        ...
        final_diff, is_cached_diff = self.handle_diff_cache_processing(
            diff_output, diff_cache, working_directory
        )
    ...
    def update_summary_from_file_status(
        self, changes_summary: Dict[str, Any], file_status: Dict[str, Any]
    ) -> None:
        """
    ...
    def finalize_response_diff_field(
        self, response: ResponseDict, final_diff: str, include_diff: bool
    ) -> None:
        """
    ...
    def broadcast_changes_summary(
        self, changes_summary: Dict[str, Any], request_id: Optional[str] = None
    ) -> None:
        """
    ...
    def estimate_complexity(self, changes_summary: Dict[str, Any]) -> str:
        ...
        lines_changed = (
            changes_summary.get("stats", {}).get("lines_added", 0) +
            changes_summary.get("stats", {}).get("lines_removed", 0)
        )
    ...
    def update_api_key_status_in_response(
        self, response: ResponseDict, api_key_status: Dict[str, Any]
    ) -> None:
        """
    ...
    def add_provider_warning_to_response(
        self, response: ResponseDict, warnings: List[str]
    ) -> None:
        """
